 ...
 * 2. 할 일 목록을 렌터링 : 로컬 스토리지에 저장된 데이터가 있는 경우 불러오기
 * 
...
function renderTodos(todos) {
  ...
  todos.forEach((todo, index) => {
    ...
    // 물론 우리가 배운대로 외 부에 구현해놓고 여기서 호출만 해도 되는데 메커니즘을 설명하기 위해 여기에 작성하겠습니다.

    deleteBtn.onclick = () => removeTodo(index);  // 버튼을 클릭하면 removeIndex() 함수를 호출하겠다
    // argument로 index를 받았습니다 -> 즉 텅적 요소 하나만 삭제하는 버튼이라고 볼 수 있겠습니다.
    // deleteBtn.addEventListener('click', removeTodo(indesx)); 와 동의어
    // deleteBtn.addEventiListener('click', (function() {
    //      삭제하는 구현부; 와 같은 방식으로는 이전에 작성해왔습니다.

    deleteBtn.classList.add('deleteBtn');

    li.appendChild(deleteBtn);    // ul > li > button
    list.appendChild(li);       // ul < 1i
  });
}
